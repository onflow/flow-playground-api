...
import (
	...
	"github.com/dapperlabs/flow-playground-api/server/router"
...
func main() {
	...
	routerAPI := router.InitializeRouter()

	logStartMessage()

	log.Printf("Connect to http://localhost:%d/ for GraphQL playground", config.GetConfig().Port)
	log.Fatal(http.ListenAndServe(fmt.Sprintf(":%d", config.GetConfig().Port), routerAPI))
}
